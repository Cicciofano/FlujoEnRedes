    ...
    def ArchivoGnu(self):
        with open("tarea2.plot","w") as archivo:
            ...
            for a in range (len(self.aristas)):
                ...
                w=self.pesos[a]
                if dirigido is 1:
                    print("set arrow {:d} from {:f}, {:f} to {:f}, {:f} head filled size 0.5,10 lw 1".format(num,x1,y1,x2,y2),file=archivo)
                    if pesos is 1:
                        print("set label '{:d}' at {:f},{:f}".format(w, xm, ym), file = archivo)                        
                else:
