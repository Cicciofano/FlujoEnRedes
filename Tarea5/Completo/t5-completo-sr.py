...
class Grafo:  
    ...
    def NuevosNodos(self):
        with open ("cuadricula.dat", "w") as crear:
            ...
            if q not in self.lqq:
                self.lqq.append(q)
                #print(self.nodos[q])
                for qq in self.nodos:
                    if (self.nodos[qq], self.nodos[q]) in self.aristas:
                        del self.aristas[(self.nodos[qq], self.nodos[q])]
                    if (self.nodos[q], self.nodos[qq]) in self.aristas:
                        del self.aristas[(self.nodos[q], self.nodos[qq])]
                del self.nodos[q]
                #print(self.aristas)
            #print("quita")
    ...
    def FFN(self): # algoritmo de Ford y Fulkerson
        self.s = self.nodos[1]
        ...
        while True:
            ...
            while u in aum:
                ...
                u = v
            maximo += incr
        with open ("flujosmaximosNodos.csv", "at") as crear: 
            print(maximo, file=crear)
        return maximo

    def FFA(self): # algoritmo de Ford y Fulkerson
        self.s = self.nodos[1]
        self.t = self.nodos[k*k]
        if self.s == self.t:
            return 0
        ...
        while True:
            ...
            while u in aum:
                ...
                u = v
            maximo += incr
        with open ("flujosmaximosAristas.csv", "at") as crear: 
            print(maximo, file=crear)
        return maximo
    ...
    def percolaAristas(self):
        self.qep = floor((len(self.aristas))/10)
        self.lqq = []
...
for rep in range(1,11):
    print(rep)
    if QuitaNodos == 1:
        with open ("TiempoPercolaNodo.csv", "at") as crear:
            TI = time.clock()
            G=Grafo(k,p)
            G.CreaNodos()
            G.CreaAristas(l)
            #G.FFN()
            G.percolaNodos()
            print(time.clock()-TI, file = crear)
    if QuitaAristas == 1:
        with open ("TiempoPercolaArista.csv", "at") as crear:
            TI = time.clock()
            G=Grafo(k,p)
            G.CreaNodos()
            G.CreaAristas(l)
