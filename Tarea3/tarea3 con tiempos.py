...
class Grafo:
    ...
    def CreaAristas(self):
        if dirigido is 0:
            with open ("ArcosNoDirigidos.dat", "at") as crear:
                t=0
            ...
            with open ("ArcosDirigidos.dat", "at") as crear:
                t=0
                for(x2,y2) in self.nodos[0:self.n+1]:
                    print(x2,y2,t,file=crear)
                t=1
...
for nn in range(1, 11):
    for rr in range(0,30):
        if dirigido is 0:
            with open("tiemposND.csv","at") as archivo:
            ...
            with open("tiemposD.csv","at") as archivo:
                start_time = time.clock()
                Nnodos = nn*10    
                G = Grafo()
                G.CreaNodos()
